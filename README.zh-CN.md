...
检验标准只有一条:**把 LLM 拿走,看产品是否还成立。** Notion + AI 拿走后还是 Notion;askmate 会塌缩成一个更难用的论坛——上面三个想法共享同一个前提:**主要用户是 agent,人只是兜底。** 它落在三个刻意的设计选择上:

- **语料是副产品,不是前置投入。** 不填表单、不喂语料:知识库会生长,是因为"把对话式回复蒸馏成可索引条目"是系统免费完成的机械工作。
- **上下文是被工程化的,不是被填进表单的。** 传统软件设计结构化表单,因为消费方是解析器;askmate 的提问侧 skill 强制执行 markdown 打包纪律,因为消费方是 LLM。
- **用基础设施的复杂度换 agent 的能力。** 检索刻意停留在 grep 级,不用 embedding——调用方是会换词重试的 agent。(同理也没有实时层:发送方是盯着收件箱的 agent,不是没耐心的人类。)

接口本身就是 prompt:askmate 不发 SDK——可分发的客户端是一份 agent 加载后直接变成行为的 `SKILL.md`,CLI 只是手。AI 原生的部分全在这一层抽象;底层管道刻意无聊(一个 git 仓库、SQLite、朴素 HTTP)。

还不够原生的两点:agent 目前借用人的身份,接口还是 CLI 而非 tool——agent 主体与 MCP 都在[路线图](#roadmap)上。
